...
function createContract()
    ...
    ContractSection(
        ref_version = w.ref_version,
        contract_revision = cr,
        contractpartnerref_revision = cprr,
        ref_entities = (Dict(p.id => psection)),
    )
end
...
function renderhnode(node, i)
    Html.li() do
        """ Version $(string(node.interval.ref_version.value)) 
            <a href="csection?history_id=$(node.interval.ref_history.value)&version_id=$(node.interval.ref_version.value)"> show section</a>
        """ *
